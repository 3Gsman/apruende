...
<html lang="es">
  <head>
    <title>Propuesta enviada - aprUEnde</title>
      ...
      <div class="row">
        ...
        <div class="col-12 col-md-8">
          ...
          <div class="row">
            ...
            <div class="col-sm-4">
              <div class="btnAdd">
                <button onclick="location.href='propuestas.php'" type="button" class="btn btn-outline-dark">Todos las propuestas</button>
              </div>
            </div>
          </div>
          ...
          <div class="propSent">
            ...
            <p>Nombre: <?php echo $_POST["namePropForm"]; ?></p>
            <p>Descripcion: <?php echo $_POST["descPropForm"]; ?></p>
            <p>Facultad: <?php echo $_POST["facultPropForm"]; ?></p>
            ...
            $namePropForm = stripslashes($_REQUEST['namePropForm']);
            $namePropForm = mysqli_real_escape_string($con,$namePropForm); 
            $descPropForm = stripslashes($_REQUEST['descPropForm']);
            $descPropForm = mysqli_real_escape_string($con,$descPropForm);
            $facultPropForm = stripslashes($_REQUEST['facultPropForm']);
            $facultPropForm = mysqli_real_escape_string($con,$facultPropForm);

            $IDOwn = 1;
            $Active = 1;

            $query = "INSERT into `propuesta` (IDOwn, Name, Description, facultad, Active) VALUES ('$currentID','$namePropForm', '$descPropForm', '$facultPropForm', '$Active')";

            $result = mysqli_query($con,$query);
            if($result){
              echo "<div class='verifPropForm'>
			<h4>Se ha enviado correctamente.</h4></div>";          
            }else{
              echo "<div class='verifPropForm'>
			<h4>No ha podido ser enviada. Se ha producido un error.</h4></div>";  
            }
